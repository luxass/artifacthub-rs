...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_template_returns_matching_template() {
        ...
        let result = handle_get_template(
            &server,
            GetTemplateParams {
        ...
        assert_eq!(result.0.name, "templates/service.yaml");
        assert_eq!(result.0.data, "apiVersion: v1\nkind: Service\n");
    }

    #[tokio::test]
    async fn test_get_template_returns_error_when_missing() {
        let mock_server = MockServer::start().await;
        ...
        assert_eq!(
            result.err().as_deref(),
            Some("Template 'templates/deployment.yaml' not found")
        );
    }
}
